 ...
 * The legacy heuristic/table parser is deliberately not used here.
 */
@Service
        ...
        try (PDDocument document = Loader.loadPDF(bytes)) {
            ...
            RecruitmentExtractionResult result = aiClient.extract(extractionText)
                    .orElseThrow(() -> new IllegalStateException(
                            "Gemini extraction failed. Verify MEDEX_AI_ENABLED, MEDEX_AI_API_KEY, " +
                            "MEDEX_AI_CHAT_COMPLETIONS_URL and MEDEX_AI_MODEL on the VPS, then retry."
                    ));
