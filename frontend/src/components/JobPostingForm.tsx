...
export interface ParsedNoticeResult extends Partial<JobFormData> {
  departmentsList?: ParsedDepartmentVacancy[];
  isMultiDepartment?: boolean;
  selectionProcess?: string;
}

function parseRawVacancyNotice(rawText: string): ParsedNoticeResult {
...
export function JobPostingForm({ onCancel, onSave, initialData }: JobPostingFormProps) {
  ...
  const handlePublishMultiDepartmentRecruitment = async () => {
    if (!detectedRecruitment || detectedRecruitment.departments.length === 0) return;
    setPublishingRecruitment(true);
    try {
      const rawTitle = formData.title.trim() || detectedRecruitment.title || 'Medical Staff';
      ...
      const payload: ManualRecruitmentPayload = {
        ...
        totalVacancies: formData.numberOfPosts || detectedRecruitment.totalVacancies,
